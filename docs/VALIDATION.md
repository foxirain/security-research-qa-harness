...
| --- | --- | --- |
| Standard-library regression suite | **40 / 40 passed** | Software and safety contracts covered by `tests/` |
...
| Fresh virtual-environment install | Passed | The built wheel imports as version `0.2.1` and exposes `security-qa-harness --help` |
...
| Operational failure handling | Passed | Missing commands and test runners fail closed, produce no semantic claim verdict, and skip variants |
| Claim evidence separation | Passed | Original claim text and harness-generated inputs cannot substantiate the claim |
| Repository source filtering | Passed | Generated output, dependency, build, cache, and virtual-environment trees are excluded from path ranking |
...
repository. The installed package reported `0.2.1` and rendered CLI help.
